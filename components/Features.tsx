...
const Features = () => {
    ...
    return (
        <div className='w-full bg-black md:h-[60%] lg:h-[40rem] xl:h-[60%] h-[100%] items-center flex justify-center'>
            <div className='flex  md:flex-row flex-col items-center gap-4 bg-black h-[100%] justify-center w-[90%] mx-auto'>
                {
                    FaeturesArray.map((feature, index) => (
                        <div key={index} className={`flex md:h-[18rem] xl:h-[53%] h-[40%] py-4 text-center text-transparent bg-gradient-to-l from-[#1A1B23] via-black to-[#006270] shadow  justify-center px-10 rounded-[20px] shadow border-2xl  m-4 flex-col items-center gap-2 md:gap-4 w-[80%] md:w-[40%] xl:w-[25%]`}>
                            ...
                            <div className='text-white text-[0.8rem] w-full'>{feature.description}</div>
                        </div>
                    ))
                }
            </div>
